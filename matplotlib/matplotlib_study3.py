...
# 이미지 좌표 가져오기
ax = plt.gca()         # 전체 영역 가져오기

for (x,y,w,h,c) in pos:
# 박스 좌표 설정
...
# 사각형 박스 만들기
    box = patches.Rectangle(
        ...
        edgecolor=c, # 박스 테두리 색
        linewidth=2     # 테두리 선 굵기
    )
    ax.add_patch(box)
    # 바운딩 박스 위에 텍스트 넣기
    plt.text(x,y-10,'object',color='red',fontsize=10,bbox=dict(facecolor='white',edgecolor='red',pad=1))
    # plt.axis('off')
plt.show()
