...
# # 과일 데이터 
fruits = [ 
    {"name":"사과", "price":3000,  "stock":3 },
    {"name":"바나나", "price":1500, "stock":2 },
...
while True:
    ...
    if menu == "1":
        ...
        for fruit in fruits:
            if fruit["name"] == name:
                ...
                if fruit["stock"] >= count:
                    ...
                    sales.append(sale)
                    print("판매 완료")
                    print("총 금액 : ",fruit["price"]*count)
                else:
                    print("재고가 부족합니다.")
        if not found: # 과일 데이터에서 판매할 과일이 없는 경우  - not 연산자에 의해 거짓 <-> 참
            print("해당 과일이 없습니다.")

    elif menu == "4":
        isempty = False
        # 과일들의 재고량을 확인하여 5개 이하인 과일들을 찾아서 출력하기
        # 만약 5개이하 인 과일이 없다면 재고 부족 과일이 없습니다. 출력
        fruit = [x for x in fruits if x["stock"] <= 5 ]
        if not fruit:
            print("재고 부족 과일이 없습니다.")
        else:
            names = [f["name"] for f in fruit]
            print(f"재고 부족 과일 : {names}")
            
                

    elif menu == "5":
        # 오늘의 추천 과일 - 랜덤
        # import random - randint(1,10) - 1~10 정수 , randrange(1,10) - 1~9 정수
        ...
        fruit = [x["name"] for x in result ]
        print(f"오늘의 추천 과일 : {fruit}")
        
    elif menu == "6":
        # 판매 기록을 출력 - sales
        # 판매한 과일에 대해 출력 , 총 판매 금액 출력
        names = [s[0] for s in sales]
        total = sum(s[2] for s in sales)
        print("판매한 과일 : ",','.join(names))
        print("총 판매 금액 : ",total)
